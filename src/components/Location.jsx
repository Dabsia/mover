...
//   const isMapLoaded = isLoaded && !!placesLib && !!geocodingLib;
...
//         if (
//           status === geocodingLib.GeocoderStatus.OK &&
//           results &&
//           results[0]
//         ) {
...
//       });
//     },
//     [placesLib, setErrMessage, setProvinceId]
//   );
...
//     }
//   }, [fromLocation, isEditingFrom, isMapLoaded, validateAddress]);
...
//       if (fromLocation && fromLocation.length >= 3 && isMapLoaded) {
...
const Location = ({
  ...
  const searchPlaces = useCallback(async (query, type = "from") => {
    ...
    const setLoading = type === "from" ? setIsLoadingFrom : setIsLoadingTo;
    ...
    try {
      // Use Geoapify autocomplete endpoint
      const url = `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(query)}&apiKey=${GEOAPIFY_API_KEY}&limit=10&format=json`;
      ...
      if (data.results && data.results.length > 0) {
        // Transform Geoapify results to match the expected format
        const predictions = data.results.map((result) => ({
          place_id: result.place_id || `place-${Math.random()}`,
          description: result.formatted || result.address_line1 || result.address_line2 || result.street,
          structured_formatting: {
            main_text: result.street || result.city || result.address_line1 || result.formatted || "",
            secondary_text: result.city ? `${result.city}, ${result.state}` : result.country || ""
          },
  ...
  // Get place details and coordinates using Geoapify
  const getPlaceDetails = useCallback(async (placeId, type = "from") => {
    try {
      // First try to get details using the place_id
      const url = `https://api.geoapify.com/v2/places/${placeId}?apiKey=${GEOAPIFY_API_KEY}`;
      ...
      if (data.features && data.features.length > 0) {
        ...
        return {
          ...
          lng,
          address_components: {
            province: properties.state || properties.region || "",
  ...
  const geocodeAddress = useCallback(async (address) => {
    try {
      const url = `https://api.geoapify.com/v1/geocode/search?text=${encodeURIComponent(address)}&apiKey=${GEOAPIFY_API_KEY}&format=json&limit=1`;
      ...
      if (data.results && data.results.length > 0) {
        ...
        return {
          lat: result.lat,
          lng: result.lon,
          province: result.state || result.region || "",
  ...
  // Validate address and extract province
  const validateAddress = useCallback(async (address) => {
    ...
    try {
      ...
      if (geocodeResult) {
        console.log("Geocoding results:", geocodeResult);

        // Set coordinates
        setFromPickupLatitude(geocodeResult.lat);
        setFromPickupLongitude(geocodeResult.lng);

        const extractedProvinceName = geocodeResult.province;

        if (extractedProvinceName) {
          console.log("Setting province name:", extractedProvinceName);
          setProvinceName(extractedProvinceName);

          if (allProvinces?.result?.length > 0) {
            const matchedProvince = allProvinces.result.find(
              (p) => p.provinceName.toLowerCase() === extractedProvinceName.toLowerCase()
            );
      ...
      setProvinceId(null);
      setErrMessage("Error validating address");
    }
  }, [allProvinces, setErrMessage, setProvinceName, setProvinceId, setFromPickupLatitude, setFromPickupLongitude, geocodeAddress]);
  ...
  useEffect(() => {
    ...
    if (fromLocation && fromLocation.length >= 3 && !isEditingFrom) {
      console.log("Triggering validation for:", fromLocation);
      validateAddress(fromLocation);
    }
  }, [fromLocation, isEditingFrom, validateAddress]);
  ...
  const selectFromSuggestion = useCallback(async (prediction) => {
    console.log("Selecting from suggestion:", prediction.description);
    setFromLocation(prediction.description);
    setErrMessage("");
    setIsEditingFrom(false);
    ...
    try {
      ...
      const placeDetails = await geocodeAddress(prediction.description);

      if (placeDetails) {
        const { lat, lng, province } = placeDetails;
        ...
        if (province) {
          console.log("Setting province name from suggestion:", province);
          ...
          if (allProvinces?.result?.length > 0) {
            ...
            if (matchedProvince) {
              console.log("Setting province ID from suggestion:", matchedProvince.provinceId);
              setProvinceId(matchedProvince.provinceId);
              setErrMessage("");
            } else {
              setProvinceId(null);
      ...
      setProvinceId(null);
      setErrMessage("Error selecting address");
    }
  }, [allProvinces, geocodeAddress, setErrMessage, setFromLocation, setFromPickupLatitude, setFromPickupLongitude, setProvinceId, setProvinceName]);

  const selectToSuggestion = useCallback(async (prediction) => {
    console.log("Selecting to suggestion:", prediction.description);
    setToLocation(prediction.description);
    setErrMessage("");
    setIsEditingTo(false);
    ...
    try {
      ...
      if (placeDetails) {
        ...
        console.log("Failed to get coordinates for to location");
      }
    } catch (error) {
      console.error("Error selecting to suggestion:", error);
    }
  }, [geocodeAddress, setToDropOffLatitude, setToDropOffLongitude, setToLocation]);
  ...
  // Modal and move size code remains the same
  const modalRef = useRef(null);
  const inputRef = useRef(null);
  const [showSizeModal, setShowSizeModal] = useState(false);
  const houses = [
                ...
                {showSizeModal && (
                  ...
                  >
                    <div className="p-3 flex items-center border-b-[1px] justify-between">
                      <button
                        onClick={() => setActiveMoveSizeTab("house")}
                        className={`${activeMoveSizeTab === "house"
                            ? "bg-[#F0F9FD] border-[1px]"
                            : ""
                          } w-[100px] p-2 rounded-[8px] border-[#BCDFF6] flex items-center`}
                      >
                      ...
                      <button
                        onClick={() => setActiveMoveSizeTab("apartment")}
                        className={`${activeMoveSizeTab === "apartment"
                            ? "bg-[#F0F9FD] border-[1px]"
                            : ""
                          } w-[150px] p-2 rounded-[8px] border-[#BCDFF6] flex items-center`}
                      >
                      ...
                      <button
                        onClick={() => setActiveMoveSizeTab("storage")}
                        className={`${activeMoveSizeTab === "storage"
                            ? "bg-[#F0F9FD] border-[1px]"
                            : ""
                          } w-[100px] p-2 rounded-[8px] border-[#BCDFF6] flex items-center`}
                      >
